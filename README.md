...
We will use [Santander Smart City](http://datos.santander.es/dataset/?id=datos-trafico) data to ingest into our system and play around. This web has a [JSON option](http://datos.santander.es/api/rest/datasets/mediciones.json?items=482) to retrieve data in the form of a JSON document through API REST, allowing us to ingest 482 individual traffic measures each http GET/POST call.
